    ...
    public class FValidaciones
    {
        ...
        public class EMail
        {
            public static string Verificar(string pEmailAddress)
            {
                ...
                try
                {
                    addr = new System.Net.Mail.MailAddress(pEmailAddress);
                }
                catch (Exception)
                {
            ...
            public static bool Valido(string pEmailAddress)
            {
                ...
                */
                string pattern = "^[_a-zA-Z0-9-]+(\\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\\.[a-zA-Z0-9-]+)*\\.(([0-9]{1,3})|([a-zA-Z]{2,3})|(aero|coop|info|museum|name))$";
                Match emailAddressMatch = Regex.Match(pEmailAddress.Trim(), pattern, RegexOptions.IgnoreCase);
        ...
        {
            int num;
            bool result = Int32.TryParse(pStr, out num);
        ...
        public static string FechaDesdeHastaCorrectas(string pFechaDesde, string pFechaHasta)
        {
            ...
            if (FFechas.FechaComoDate(pFechaDesde) > DateTime.Now.Date)
            {
                respuesta += FTextos.FechaDesdeImposible();
            }
            if (FFechas.FechaComoDate(pFechaHasta) < DateTime.Now.Date)
            {
                respuesta += FTextos.FechaHastaImposible();
            }
            return respuesta;
        ...
        {
            string respuesta = "";
            int iDesdeMes = int.Parse(pDesdeMes);
            if (iDesdeMes < 1 || iDesdeMes > 12)
            {
                respuesta += FTextos.FechaMesErrorDesde();
            }
            if (pDesdeAnio.Length == 4)
            {

                int iDesdeAnio = int.Parse(pDesdeAnio);
                if (iDesdeAnio < 1970)
                {
            ...
            int iHastaMes = int.Parse(pHastaMes);
            if (iHastaMes < 1 || iHastaMes > 12)
            {
                respuesta += FTextos.FechaMesErrorHasta();
            }
            if (pHastaAnio.Length == 4)
            {

                int iHastaAnio = int.Parse(pHastaAnio);
                if (iHastaAnio < 1970)
                {
