...
namespace FuncionesCore
{
    public class FMails
    {
        ...
        {
            SmtpClient client = new SmtpClient(pSMTP);
            client.UseDefaultCredentials = false;
            client.Credentials = new System.Net.NetworkCredential(pCuentaDeEnvio, pPwdDeEnvio);
            client.DeliveryMethod = SmtpDeliveryMethod.Network;

            MailMessage mail = new MailMessage(pCuentaDeEnvio, pCuentaDeDestino);
            ...
            if (pPathAdjunto != "")
            {
                Attachment Att = new Attachment(pPathAdjunto);
                mail.Attachments.Add(Att);
            }
            if (pCuentasDeDestinoCC != "")
            {
                mail.CC.Add(pCuentasDeDestinoCC);
            }
            if (pCuentasDeDestinoCCO != "")
            {
                mail.Bcc.Add(pCuentasDeDestinoCCO);
            }
        ...
        public static void EnviarmailAsync(string pCuentaDeEnvio,
                                    ...
                                    string pCuentasDeDestinoCCO = "")
        {
            Thread thread = new Thread(() => Enviar(pCuentaDeEnvio,
                                    pPwdDeEnvio,
                                    pCuentaDeDestino,
                                    pAsunto,
                                    pMensaje,
